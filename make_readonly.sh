...
# Get the UUID of the data partition for reliable mounting.
DATA_UUID=$(blkid -s UUID -o value "$DATA_PART")
if [[ -z "$DATA_UUID" ]]; then
    die "Could not read UUID of $DATA_PART."
fi
...
if [[ -z "$ROOT_UUID" ]]; then
    die "Could not read UUID of $ROOT_PART."
fi

# Remove any existing entries for / that we will replace.
# Keep all other entries (boot partition, etc.).
FSTAB_NEW=$(grep -v "^UUID=$ROOT_UUID\|^[^#]*\s/\s" /etc/fstab || true)

cat > /etc/fstab << EOF
# RaspberryFluke fstab — generated by make_readonly.sh
# Original backed up at /etc/fstab.bak

# Boot partition — read-write (required for firmware updates)
$(grep "/boot" /etc/fstab.bak || echo "# (no boot entry found in original fstab)")

# Root filesystem — READ ONLY
# This protects the SD card from corruption on hard power cuts.
UUID=$ROOT_UUID  /  ext4  ro,noatime  0  1

# Writable data partition for port history and debug logs.
UUID=$DATA_UUID  /data  ext4  rw,noatime  0  2
...
if [[ -n "$CMDLINE_FILE" ]]; then
    ...
    if ! grep -q "\bro\b" "$CMDLINE_FILE"; then
        # Add 'ro' before 'rootwait' if present, otherwise append.
        if grep -q "rootwait" "$CMDLINE_FILE"; then
            sed -i 's/rootwait/ro rootwait/' "$CMDLINE_FILE"
        else
            sed -i 's/$/ ro/' "$CMDLINE_FILE"
        fi
        info "Read-only root added to $CMDLINE_FILE."
    else
        info "Read-only root already in $CMDLINE_FILE."
    fi
else
